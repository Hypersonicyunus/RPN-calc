# THIS REPOSITORY IS NOT FINISHED YET! THERE IS NO WORKING PRODUCT! DONT EVEN THINK ABOUT COPYING THE SOURCE CODE AND RUNNING CARGO, IT GENUINLEY WILL NOT DO ANYTHING RELATED TO RPN ;-; (Scroll down to see the readme that part is almost done)



# Reverse Polish Notation Calculator CLI Tool
Hey there! Thanks for checking out my cli tool! This .readme is going to be a guide for you to help understand what the tool is and how to use/install it.

# What is Reverse Polish Notation (RPN)?
RPN is a way to **express a mathematic equation without the need of brackets.** It works by using writing tokens, which are operands (numbers) and operators (+,-,*,/).

# How is RPN calculated?
RPN is calculated by using the first two tokens (that are operands) before an operator, and calculating them like you usually would. Here is a visual example:

<img width="731" height="530" alt="image" src="https://github.com/user-attachments/assets/56132af9-c03e-4a3f-8aa7-80510cc593f6" />
